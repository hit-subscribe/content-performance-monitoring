CREATE VIEW @client.ga4 AS SELECT date, page as url, clicks as screenpageviews, 'organic' as sessionsourcemedium FROM @client.gsc WHERE clicks > 0;
...
SELECT 
  FORMAT_TIMESTAMP('%Y-%V', date) AS year_week,
...
CREATE VIEW @client.refresh_candidates AS SELECT 
  cm.url, 
  up.lastmod,
...
GROUP BY cm.url, r.total_views, up.projected_traffic, underperformance, up.lastmod
