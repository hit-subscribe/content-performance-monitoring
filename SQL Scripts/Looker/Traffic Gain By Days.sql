SELECT 
    ...
    SUM(CASE WHEN DATE(date) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL @days DAY) AND CURRENT_DATE() THEN screenpageviews ELSE 0 END) -
    	SUM(CASE WHEN DATE(date) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL (@days*2) DAY) AND DATE_SUB(CURRENT_DATE(), INTERVAL (@days+1) DAY) THEN screenpageviews ELSE 0 END) AS gain
FROM 
    @client.ga4
